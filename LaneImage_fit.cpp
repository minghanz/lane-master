...
void LaneImage::__fitLaneMovingWindow(int &hist_width, bool &last_all_white, VehMask &veh_masker)
{
	...
	cout << to_string(((float)(t_temp3 - t_temp1)) / CLOCKS_PER_SEC) << "s. " << endl
		 << endl;
	...
	for (int iteration_subsample = 0; iteration_subsample < iteration_subsample_num; iteration_subsample++)
	{
		...
		if (iteration_subsample != iteration_subsample_num - 1)
		{
			...
			left_fit = (Y_lane(Range(0, 3), Range(0, length_left)) * Y_lane(Range(0, 3), Range(0, length_left)).t() + regularizer_left).inv() * Y_lane(Range(0, 3), Range(0, length_left)) * (X_lane.colRange(0, length_left).t());
			right_fit = (Y_lane(Range(0, 3), Range(length_left, length_left + length_right)) * Y_lane(Range(0, 3), Range(length_left, length_left + length_right)).t() + regularizer_right).inv() * Y_lane(Range(0, 3), Range(length_left, length_left + length_right)) * (X_lane.colRange(length_left, length_left + length_right).t());
			...
			res_lane.rowRange(0, length_left) = (abs(X_lane.colRange(0, length_left) - left_fit.at<float>(2) * Y_lane(Range(2, 3), Range(0, length_left)) - left_fit.at<float>(1) * Y_lane(Range(1, 2), Range(0, length_left)) - left_fit.at<float>(0))).t() + 0.1;
			res_lane.rowRange(length_left, length_left + length_right) = (abs(X_lane.colRange(length_left, length_left + length_right) - left_fit.at<float>(2) * Y_lane(Range(2, 3), Range(length_left, length_left + length_right)) - left_fit.at<float>(1) * Y_lane(Range(1, 2), Range(length_left, length_left + length_right)) - left_fit.at<float>(0))).t() + 0.1;
		...
		else
		{
			...
			Mat lanelineboard(warp_row, warp_col, CV_8UC1, Scalar(0));
			polylines(lanelineboard, __plot_pts_lr_warp, false, Scalar(255), 1, LINE_4);
			vector<Mat> channels;
			split(__lane_window_out_img, channels);
			channels[1] = lanelineboard + channels[1] + 0;
			merge(channels, __lane_window_out_img);
		...
		if (__branch_grow_count + 1 > 0)
		{
			...
			if (__branch_at_left)
			{
				left_lane_inds = (nonzy < window_height * (__window_number - actual_branch_count / 3)) & (nonzx > (left_fy - fcol / 2) * ((frow - 1 - nonzy) * __k_pitch + __b_pitch) + fcol / 2 - __window_half_width * decay_coeff) & (nonzx < (left_fy - fcol / 2) * ((frow - 1 - nonzy) * __k_pitch + __b_pitch) + fcol / 2 + __window_half_width * decay_coeff);
				right_lane_inds = (nonzx > (right_fy - fcol / 2) * ((frow - 1 - nonzy) * __k_pitch + __b_pitch) + fcol / 2 - __window_half_width * decay_coeff) & (nonzx < (right_fy - fcol / 2) * ((frow - 1 - nonzy) * __k_pitch + __b_pitch) + fcol / 2 + __window_half_width * decay_coeff);
			}
			else
			{
				right_lane_inds = (nonzy < window_height * (__window_number - actual_branch_count / 3)) & (nonzx > (right_fy - fcol / 2) * ((frow - 1 - nonzy) * __k_pitch + __b_pitch) + fcol / 2 - __window_half_width * decay_coeff) & (nonzx < (right_fy - fcol / 2) * ((frow - 1 - nonzy) * __k_pitch + __b_pitch) + fcol / 2 + __window_half_width * decay_coeff);
				left_lane_inds = (nonzx > (left_fy - fcol / 2) * ((frow - 1 - nonzy) * __k_pitch + __b_pitch) + fcol / 2 - __window_half_width * decay_coeff) & (nonzx < (left_fy - fcol / 2) * ((frow - 1 - nonzy) * __k_pitch + __b_pitch) + fcol / 2 + __window_half_width * decay_coeff);
			}
		}
		else
		{
...
void LaneImage::trainmodel(Mat &warped_filter_image_U, valarray<float> &nonzx, valarray<float> &nonzy, valarray<bool> &left_lane_inds, valarray<bool> &right_lane_inds)
{
	...
	valarray<bool> close_left_inds = (notlane_x_pre > __left_fit[2] * (frow - 1 - notlane_y_pre) * (frow - 1 - notlane_y_pre) + __left_fit[1] * (frow - 1 - notlane_y_pre) + __left_fit[0] - 2 * __window_half_width) & (notlane_x_pre < __left_fit[2] * (frow - 1 - notlane_y_pre) * (frow - 1 - notlane_y_pre) + __left_fit[1] * (frow - 1 - notlane_y_pre) + __left_fit[0] + 2 * __window_half_width);
	valarray<bool> close_right_inds = (notlane_x_pre > __right_fit[2] * (frow - 1 - notlane_y_pre) * (frow - 1 - notlane_y_pre) + __right_fit[1] * (frow - 1 - notlane_y_pre) + __right_fit[0] - 2 * __window_half_width) & (notlane_x_pre < __right_fit[2] * (frow - 1 - notlane_y_pre) * (frow - 1 - notlane_y_pre) + __right_fit[1] * (frow - 1 - notlane_y_pre) + __right_fit[0] + 2 * __window_half_width);
