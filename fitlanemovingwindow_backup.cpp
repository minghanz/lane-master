...
void LaneImage::__fitLaneMovingWindow(int& hist_width, bool& last_all_white)		// date: 2017/10/19, want to add regularization and improve speed
{
	///based on the warped binary output
...
// fit function with regularization and pitch compensation 2017/10/21
void LaneImage::__fitLaneMovingWindow(int& hist_width, bool& last_all_white)
{
	...
	for (int iteration_subsample = 0; iteration_subsample < iteration_subsample_num; iteration_subsample++)
	{
		...
		regularizer.at<float>(2,2) = min(max((float)0, lefty.min()-(warp_row - lefty.max()) ), max((float)0, righty.min()-(warp_row - righty.max()) ) );
		...
		if (iteration_subsample != iteration_subsample_num - 1)
		{
			...
			for (int iteration = 0; iteration < iteration_num; iteration++)
			{
				...
				Mat k_mat = (Y_lane_ff.mul(w_lane.t())*Y_lane_ff.t()).inv()*Y_lane_ff.mul(w_lane.t())*X_lane_ff.t();
		...
		// else if (iteration_subsample == 1)	// make the second iter more similar to the first
		// {
		// 	lane_fit = (Y_lane.t()).inv(DECOMP_SVD)*(X_lane.t());
		// 	int iteration_num = 5;
		// 	// for (int iteration = 0; iteration < iteration_num; iteration++)
		...
		// 	{
		// 		Mat Y_lane_f = lane_fit.at<float>(2)*Y_lane.row(2) + lane_fit.at<float>(1)*Y_lane.row(1) + lane_fit.at<float>(0) + lane_fit.at<float>(3)*Y_lane.row(3);
		// 		Mat Y_lane_ff = (Y_lane_f - warp_col/2).mul(Y_lane.row(1));
		// 		Mat X_lane_ff = Y_lane_f - X_lane;

		// 		Mat res_lane = (abs(X_lane_ff)).t() + 0.1;
		// 		res_lane = 1/res_lane; //(frow - 1 - Y_left.row(1))large for downside
		// 		Mat w_lane = Mat(res_lane.rows, 1, CV_32F);
		// 		w_lane.col(0) = (W_lane.t()); //res_lane.col(0).mul(W_lane.t())
		...
		// 		Mat X_lane_f = (X_lane - warp_col/2)/(1-k_pitch*Y_lane.row(1)) + warp_col/2;
		// 		res_lane = (abs(X_lane_f - Y_lane_f)).t() + 0.1;
		...
		// 	__k_pitch = -k_pitch;
		// 	__b_pitch = 1;
		// 	cout << "k_pitch: " << k_pitch << ", y0: " << int(1/k_pitch) << endl;
		...
		// 		}
		// 		polylines(inishow_lane_window, plot_pts_lr_warp, false, Scalar(0, 255, 0), 1, LINE_4 );
		// 		imshow("1strun_unionfit", inishow_lane_window);
		// 	}
		// 	#endif
		...
		// 		warp_dst_comp.push_back(Point2f(warp_col , 0));
		// 		float x_shrink = -warp_row*__k_pitch*warp_col;
		// 		warp_src_comp.push_back(Point2f(x_shrink, 0));
		// 		warp_src_comp.push_back(Point2f(0, warp_row));
		// 		warp_src_comp.push_back(Point2f(warp_col, warp_row));
		// 		warp_src_comp.push_back(Point2f(warp_col - x_shrink , 0));

		// 		__inv_per_mtx_comp = getPerspectiveTransform(warp_dst_comp, warp_src_comp);
		// 	}
		// 	else if (-__k_pitch < -2e-5)
		// 	{
		// 		vector<Point2f> warp_src_comp, warp_dst_comp;
		// 		warp_dst_comp.push_back(Point2f(0, 0));
		// 		warp_dst_comp.push_back(Point2f(0, warp_row));
		...
		// // side fit currently disabled
		// Mat regularizer_left(3,3,CV_32FC1, Scalar(0));		
		// Mat regularizer_right(3,3,CV_32FC1, Scalar(0));
		// regularizer_left.at<float>(2,2) = max((float)0, lefty.min()-(warp_row - lefty.max()) );
		// regularizer_right.at<float>(2,2) = max((float)0, righty.min()-(warp_row - righty.max()) );
		// cout << regularizer_left.at<float>(2,2) << " " << regularizer_right.at<float>(2,2) << endl;
			// if (lefty.min() < warp_row/3 && righty.min() < warp_row/3 && lefty.max() - lefty.min() > warp_row/3 && righty.max() - righty.min() > warp_row/3)
			// {
			// 	int iteration_num = 2;
			// 	Mat res_lane(X_lane.cols, 1, CV_32FC1);
			// 	for (int iteration = 0; iteration < iteration_num; iteration++)
			...
			// 		Mat w_verti_pos;
			// 		sqrt((frow - 1 - Y_lane.row(1)).t(), w_verti_pos);
			// 		res_lane = 1/res_lane.mul(w_verti_pos) *0.1; //(frow - 1 - Y_left.row(1))large for downside
			// 		Mat w_lane(res_lane.rows, 3, CV_32F);
			// 		w_lane.col(0) = res_lane.col(0).mul(W_lane.t());
			// 		w_lane.col(1) = w_lane.col(0) + 0;
			// 		w_lane.col(2) = w_lane.col(0) + 0;
					
			// 		left_fit = (Y_lane(Range(0, 3), Range(0, length_left)).mul(w_lane.rowRange(0, length_left).t())*Y_lane(Range(0, 3), Range(0, length_left)).t() + regularizer_left).inv()
			// 			*Y_lane(Range(0, 3), Range(0, length_left)).mul(w_lane.rowRange(0, length_left).t())*(X_lane.colRange(0, length_left).t());
			// 		right_fit = (Y_lane(Range(0, 3), Range(length_left, length_left + length_right)).mul(w_lane.rowRange(length_left, length_left + length_right).t())*Y_lane(Range(0, 3), Range(length_left, length_left + length_right)).t() + regularizer_right).inv()
			// 			*Y_lane(Range(0, 3), Range(length_left, length_left + length_right)).mul(w_lane.rowRange(length_left, length_left + length_right).t())*(X_lane.colRange(length_left, length_left + length_right).t());
			// 	}
			// 	if (iteration_subsample == iteration_subsample_num - 1) // last iteration is different
			// 	{
			// 		__left_fit = left_fit; // Mat(1,3) to Vec3f, OK???
